...
editor_assign_needed_data <- data %>% filter(journal == this_journal) %>% #pull journal of interest
  ...
  separate(submitted.date, c("year", "month", "day"), sep = "-") %>% select(-day) %>% #determine submission year & month
  filter(version == "0") %>% filter(year == this_year) #restrict to first version submitted in specified year

#df of the number of manuscripts assigned to each editor by month
by_editor <- editor_assign_needed_data %>% 
  group_by(editor, month) %>% summarise(n = n()) %>% #pull number of manuscripts assigned to each editor, each month
...
month_sums <- by_editor %>% select(-editor) %>% colSums(., na.rm = TRUE) %>% #calculate total for each column
...
empty_months <- tryCatch(
  matrix(data = NA, nrow = nrow(month_sums), ncol = (13 - ncol(month_sums))) %>% as.data.frame(),
...
ed_sums <- month_sums %>% select(-editor) %>% sapply(., as.numeric) %>% rowSums(., na.rm = TRUE) %>% as.data.frame()

#calculate average number of manuscripts assigned per month
ed_avg <- month_sums %>% select(-editor) %>% sapply(., as.numeric) %>% rowMeans(., na.rm = TRUE) %>% 
...
editor_assign_all_summary <- mutate(editor_assign_summary, 
                                    `Percent YTD` = get_percent(`Total YTD`, 
