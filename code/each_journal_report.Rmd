...
```{r, journ_subs_cat, fig.length = 12, fig.width=8, results='asis'}
source("each_journal_monthly_report/get_journ_subs.R")
...
knitr::kable(J_recent_transfers, caption = "Transfers received in the last month",
             format = "html", digits = 2,  col.names = c("Date", "From", "Manuscript Number", "Title", "Editor", "Decision")) %>%
  kable_styling()

knitr::kable(J_transfers_ytd, 
             caption = "Final outcomes on transfers recieved in the last 12 months",
             format = "html", digits = 2,  col.names = c("Decision", "N", "Percent")) %>%
  kable_styling()
```
