...
#source("code/test_data.R")

needed_data <- clean_test_data %>% 
  select(Journal, submitted.date, days.to.decision, `Date of Publication`, manuscript.number, version) %>%
...
summary_all <- data.frame("All", summary_all_1, summary_all_2)
